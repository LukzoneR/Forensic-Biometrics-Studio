import { i18nKeywords as Dictionary } from "@/lib/locales/translation";

const d: Dictionary = {
    Theme: "Motyw",
    Language: "Język",
    Settings: "Ustawienia",
    Markings: "Adnotacje",
    Debug: "Debug",
    "Working mode": "Tryb pracy",
    Control: "Sterowanie",
    Types: "Typy",
    Remove: "Usuń",
    Add: "Dodaj",
    Tools: "Narzędzia",
    "Edit Image": "Edycja obrazu",
    Adjustments: "Dostosowania",
    About: "O aplikacji",
    Version: "Wersja",
    Description: "Opis",
    Authors: "Autorzy",
    Repository: "Repozytorium",
    "Custom Themes": "Własne motywy",
    "Edit Theme": "Edytuj motyw",
    "Theme Name": "Nazwa motywu",
    "Theme name": "Nazwa motywu",
    Colors: "Kolory",
    Save: "Zapisz",
    Light: "Jasny",
    Dark: "Ciemny",
    "Default Theme": "Domyślny motyw",
    Export: "Eksportuj",
    Import: "Importuj",
    "Themes exported successfully": "Motywy wyeksportowane pomyślnie",
    "Failed to export themes": "Nie udało się wyeksportować motywów",
    "Themes imported successfully": "Motywy zaimportowane pomyślnie",
    "Invalid themes file": "Nieprawidłowy plik motywów",
    "Failed to import themes": "Nie udało się zaimportować motywów",
    Free: "Odręczne",
    Line: "Linia",
    Color: "Kolor",
    Opacity: "Przezroczystość",
    Thickness: "Grubość",
    Undo: "Cofnij",
    Redo: "Ponów",
    Filters: "Filtry",
    FeatureVisibility: "Widoczność cech",
    Report: "Raport",
    "Report settings": "Ustawienia raportu",
    "Report ID": "Identyfikator raportu",
    "Report date and time": "Data i czas wykonania raportu",
    "Performed by": "Wykonany przez",
    Department: "Wydział",
    "Address line 1": "Adres (linia 1)",
    "Address line 2": "Adres (linia 2)",
    "Address line 3": "Adres (linia 3)",
    "Address line 4": "Adres (linia 4)",
    "Use current date and time": "Ustaw bieżącą datę i czas",
    "Generate report": "Generuj raport",
    "Report generation": "Generowanie raportu",
    "Max features": "Maks. liczba cech",
    "Include matched only": "Tylko cechy dopasowane",
    "Matched features": "Cechy dopasowane",
    "Selected features": "Cechy wybrane",
    Feature: "Cecha",
    Cancel: "Anuluj",
    "Generating...": "Generowanie...",
    "FFT Spectrum Editor": "Edytor widma (FFT)",
    "FFT Result Preview": "Podgląd wyniku FFT",
    "Loading...": "Wczytywanie...",
    "Processing...": "Przetwarzanie...",
    "Saving...": "Zapisywanie...",
    "Save as": "Zapisz jako",
    "Brush size": "Wielkość pędzla",
    Transformations: "Przekształcenia",
    Crop: "Kadrowanie",
    Preview: "Podgląd",
    Edit: "Edycja",
    Apply: "Zastosuj",
    Clear: "Wyczyść",
    "Select working mode": "Wybierz tryb pracy",
    "No marking types found for the selected working mode":
        "Nie znaleziono typów adnotacji dla wybranego trybu pracy",
    "Select a working mode to view marking types":
        "Wybierz tryb pracy, aby wyświetlić typy adnotacji",
    Parameters: "Parametry",
    "Verification result": "Wynik weryfikacji",
    "Segments count": "Liczba odcinków obrysu",
    "Outline area": "Powierzchnia obrysu",
    "Outline perimeter": "Obwód obrysu",
    "Shape coefficient": "Współczynnik kształtu",
    "Axis W1 length": "Długość osi W1",
    "Axis W2 length": "Długość osi W2",
    "Size proportion": "Proporcja wielkości",
    Grafotype: "Grafotyp",
    "Shape coefficient agreement": "Zgodność współczynników kształtu",
    "Grafotype agreement": "Zgodność grafotypów",
    "Rank correlation": "Korelacja rangowa",
    "Critical value": "Wartość krytyczna",
    significant: "istotna",
    "not significant": "nieistotna",
    "Axes W1 and W2 are required to compute size proportion and grafotype":
        "Osie W1 i W2 są wymagane do obliczenia proporcji wielkości i grafotypu.",
    "Rank correlation requires the same number of outline segments in both samples":
        "Korelacja rangowa wymaga tej samej liczby odcinków obrysu w obu próbkach.",
    "Shoeprint markings left": "Adnotacje na obrazie 1",
    "Shoeprint markings right": "Adnotacje na obrazie 2",
    Modifiers: "Modyfikatory",
    "No modifiers yet": "Brak modyfikatorów",
    Compute: "Oblicz",
    "Unique features color": "Kolor cech unikalnych",
    "Color red": "Czerwony",
    "Color green": "Zielony",
    "Report title": "Tytuł raportu",
    "Shoeprint comparison": "Porównanie automatyczne",
    "Run comparison": "Uruchom porównanie",
    "Comparison settings": "Ustawienia porównania",
    "Image resolution (DPI)": "Rozdzielczość obrazu (DPI)",
    Threshold: "Próg binaryzacji",
    Automatic: "Automatycznie",
    "Invert print and background": "Odwróć ślad i tło",
    "Region radius": "Promień obszaru",
    "Maximum rotation angle": "Maksymalny kąt obrotu",
    "Random seed": "Ziarno losowości",
    "Analysis engine": "Silnik analizy",
    "Edge points": "Punkty krawędziowe",
    Region: "Obszar",
    "Clique size": "Rozmiar kliki",
    "Rotation angle": "Kąt obrotu",
    "Overlap Q": "Pokrycie Q",
    "Overlap K": "Pokrycie K",
    "Median squared distance": "Mediana kwadratu odległości",
    "Region distances": "Odległości między obszarami",
    "Distance in Q": "Odległość w Q",
    "Distance in K": "Odległość w K",
    Difference: "Różnica",
    "Mean overlap": "Średnie pokrycie",
    "Rotation consistency": "Spójność kąta obrotu",
    "Triangle agreement": "Zgodność trójkąta",
    "Matched regions": "Dopasowane obszary",
    "Comparison figure": "Ilustracja porównania",
    Agreement: "Zgodność",
    "Strong agreement": "Wysoka zgodność",
    "Moderate agreement": "Umiarkowana zgodność",
    "Weak agreement": "Niska zgodność",
    "Include comparison in report": "Dołącz porównanie automatyczne do raportu",
    "No comparison has been run yet":
        "Nie wykonano jeszcze porównania automatycznego",
    "The analysis engine is unavailable": "Silnik analizy jest niedostępny",
    "Extraction preview": "Podgląd ekstrakcji",
    "Running comparison": "Trwa porównanie",
    "Comparison run at": "Porównanie wykonano",
    Points: "Punkty",
    Draw: "Rysuj",
    Pan: "Przesuń",
    Eraser: "Gumka",
    Shape: "Kształt",
    Round: "Okrągły",
    Oval: "Owalny",
    "Initializing...": "Inicjalizowanie...",
};

export default d;
